...
The same pass evicts any key whose repoRoot equals `localRepoRoot` (injected by `SessionDiscovery.start`): our own repo's worktrees are never foreign. Live ones are already excluded via `getSiblingKeys()`; this catches the ones the sibling manager has pruned because their directory is gone. Evicted own keys are remembered in `ownRepoKeys` and skipped at the top of the next `scan()`, exactly like sibling keys. Without that memory a removed Remote Control worktree whose transcripts had aged past the worktrees gate (default 7d, so `peekCwdInDir` no longer classifies it) but not the foreign one (14d in a typical config) was re-tracked on every rescan, painted as flat `bridge-cse_*` rows, resolved to `localRepoRoot`, evicted, and forgotten — a fold/unfold flicker in the repo's own window every tenth poll cycle. The set is cleared with the other caches when `localRepoRoot` changes.

`scan()` caches each dir's cwd right after tracking its sessions rather than after the whole walk, and `getWorkspaces()` withholds any key whose cwd is cached but whose repoRoot is not yet resolved. The intermediate state (sessions exist, root unknown) used to paint as unfolded rows with `done` chips — no fold, no unreachable-to-stale promotion — whenever a `sendUpdate()` landed mid-scan; since v1.26.0's progressive first paint that also meant every window start. The resolution lands before `scan()` returns, so a withheld row is at most one render late.
