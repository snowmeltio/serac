...
| `rcLauncher.ts` | Pure half of the indicator's click: terminal env overrides (strip `VSCODE_*`/`ELECTRON_RUN_AS_NODE`, keep `CLAUDE_CONFIG_DIR`), CLI location, the `claude rc` and `install.sh` command strings (with `--spawn worktree` only when `serac.rc.spawnWorktree` is on and a git repo exists), the quick-pick rows for whatever is off, the reuse rule for an already-open server terminal (`findLiveRcTerminal`), and the watch that notices a started server going away (`rcWatchTick`). |
...
- **Start a server here** — `vscode.window.createTerminal` in the workspace
  ...
  `serac.rc.spawnWorktree` is on and the workspace resolves to a git repo
  (`gitWorktreeUtil.ts:resolveRepoRoot`); the launchd installer gets the same
  mode. Visible, the user's to close. A terminal named
