...
| `processIdentity.ts` | `verifyClaudeProcess(pid, registryStart)`: before the transfer flow signals a pid it did not start, `ps -o etime=,command=` (under `LC_ALL=C`; elapsed time is digits-only, so no locale or DST ambiguity) must show a Claude CLI command whose implied start sits within 5 s of the registry's `procStart` (falling back to `startedAt`). A crashed child's leftover registry file plus a recycled pid otherwise reads as live. Checked before the modal and again right before the signal. |
...
4. **Extended thinking grace** — a turn that has produced **no output yet** (no text/tool_use, no active tools, turn-coherent) is not demoted by the 3-min hard ceiling. It defers to the PID-liveness check (`isProcessAlive()` = `kill(pid, 0)`): the idle timer's first fire waits ~30s from turn start (covers slow first-token), then both the idle timer and the poll-path `computeDemotion` re-arm while the process is alive and mark `done` only when it dies. A model can ruminate for **many minutes** before its first text/tool_use (e.g. before launching a Workflow), so a fixed ceiling here falsely demoted live thinking blocks to `done`. Bounded by a generous **15-min** absolute backstop (`EXTENDED_THINKING_CEILING_MS`) for the rare case where liveness can't be confirmed (no writer PID captured), so a truly hung process can't read `running` forever. The writer PID is captured via `fuser` once per session on the first **live** record processed while running, never during the startup replay of a JSONL (a dormant session's replay contains a running transition too, and N recent sessions would otherwise cost N lsof scans at window open); the registry death-gate covers liveness until then.
5. **Hard ceiling (3 min running\*, 10 min waiting)** — safety net; forces done. \*The 3-min running ceiling applies to a turn that has **produced output or holds an active tool** — a no-output extended-thinking turn instead defers to liveness (item 4). Covers laptop sleep, quota hits, abandoned permission prompts. `lastActivity` itself is clamped to wall-clock `now` (`clampToNow`, `sessionManager.ts`, applied at both the main-thread stamping site and `updateSubagentActivity`): a corrupt or clock-skewed record timestamp in the future must never be allowed to pin `lastActivity` ahead of the present, which would drive this ceiling's age computation negative and stall demotion indefinitely (audit finding, 2026-07-23).
...
- **Display (done-while-delegating gap, closed 2026-07-23)** — the machine
  deliberately leaves `status: 'done'` for the whole gap (by design, per
  **Turn end** above), so a card whose only live work was a detached
  background agent read `Done` for potentially minutes at a stretch, until the
  next `<task-notification>`/registry-death/dormant-sweep backstop resolved
  it. `SessionDiscovery.getSnapshots()` now derives a display-only override —
  a `done` snapshot with at least one `running && background` subagent
  presents as `running` (no new label, no card chrome) — mirroring the
  existing **Workflow-live status override** below for the population it
  doesn't cover: a background `Agent`/`Task` that never gets wrapped in a
  Workflow run. Applied before the stale rollover (so a delegating card is
  never rolled to `stale`) and before the two-zone sort (so it sorts into the
  active zone). Sibling snapshots are out of scope. The underlying
  machine-level demotion is untouched — this is purely presentational,
  terminated by the same backstops as before.
...
- **Backstops** — `sweepBackgroundWork()` (the per-poll dormant sweep, shared
  ...
  rather than getting a fresh 15-min grace on every reload (2026-07-23).
...
- **Clearing (three paths):** (1) a terminal retrieval clears its shell on the
  ...
  gate above). (A dormant session is only handed back to `update()` at all
  when `SessionManager.checkMtime()` reports a change or it has live
  background agents (see "Dormant pumping" above). `checkMtime()` itself
  fires on mtime moving **or** bytes remaining unread past `JsonlTailer`'s
  `MAX_READ_PER_CYCLE` cap — `updateInner()` loops internally to drain an
  oversized (>16 MB) transcript fully within one `update()` call, so in the
  ordinary case the tailer is always caught up by the time `checkMtime()`
  runs; the size clause is the safety net for the one way that can still
  leave unread bytes — a read that fails partway through the drain — without
  which a dormant classification taken right after would strand the rest of
...
- **Start a server here** — `vscode.window.createTerminal` in the workspace
  ...
  sessions here), then `claude rc` (preferring `~/.local/bin/claude`).
  Same-dir by default: phone-started sessions land under this workspace's
  project key, which is the only key the Claude Code panel here can restore
  a session from. `--spawn worktree` is added only when
  `serac.rc.spawnMode` is `worktree`, or `auto` and the workspace resolves
  to a git repo (`gitWorktreeUtil.ts:resolveRepoRoot`); the launchd installer
  gets the same mode. Visible, the user's to close. A terminal named
...
- **The server process itself never registers.** Presence is inferred from the
  ...
  has not yet spawned anything is invisible. The default invocation pre-creates
  a session in the serving directory, so in practice server-up implies at least
  one matching registry entry.
...
3. `rc-idle` only: first `processIdentity.verifyClaudeProcess()` — the registry
   ...
   (not a Claude command, start time off by >5 s, no start stamp, ps failed)
   refuses without offering the modal. Then the modal "Bring this phone
   session here?" naming that the phone's copy ends. Re-read the gate,
   re-classify, and **verify the identity again** after the modal (it can sit
   open long enough for the child to die and its pid to be reused); the
   target must still be
...
4. `rewriteTranscriptEntrypoint()`: stream (bounded to the size at start) to
   ...
   real key.
5. Steps 4 and 5 run as one unit inside the session's update chain
   (`discovery.reloadSessionAfter()` → `SessionManager.runThenReplay()`): the
   rewrite executes on the chain, then — if anything changed — the tailer is
   reset and the transcript replayed. The file grew in place (each record +6
   bytes) and `JsonlTailer` only self-resets on a shrink; and the rename bumps
   the mtime, which wakes the poll loop, so a poll `update()` queued between
   a separate rename and replay would read the grown file from the old offset
   (a torn line, then already-processed records re-run as live). On the chain
   nothing can read between the write and the reset. Titles survive the reset
...
A second on-disk file, separate from `session-meta.json` above and shared by
every window, VS Code profile, and farmed account on the machine: one JSON
blob keyed by each session's absolute JSONL path (`replayCache.ts`). Purpose:
a dormant `done` session whose file hasn't changed since it was last read
shouldn't be replayed from byte 0 again just because a new window opened.

**Eligibility (writing an entry) — `SessionManager.exportCachedState(now)`.**
Deliberately conservative: refuses unless the session is `done`, has no
active tools, no running subagent (including a live `run_in_background`
agent — the card can read `done` while one keeps working, see "Detached
(run_in_background) agents" above), no outstanding background shell, no
pending `ScheduleWakeup`/session cron, isn't mid-compaction, its tailer has
...
**Wiring.** `SessionDiscovery` constructs one `ReplayCacheStore` (always —
construction is cheap) at `path.join(path.dirname(projectsDir), 'serac-
replay-cache.json')`, the same directory the sessions registry sits next to
...
transcripts, defeating the whole point of a machine-wide cache. `maxAgeMs` is
the widest of every configured discovery gate (worktrees / foreign
workspaces / teams / workflows / the fixed local `SCAN_AGE_GATE_MS` /
`foreignWindowGate().ageGateMs`), clamped to the store's own 60-day hard
ceiling, plus one day of slack — a session still visible in ANY section
should never have its cache entry pruned out from under it. The store is
injected into `ForeignWorkspaceManager` and `SiblingWorktreeManager` via
`setReplayCache()`, the same one-shot pattern as `setLivenessProbeFactory()`.

`serac.discovery.replayCache` (default **on**) is the kill switch, checked at
every point of use rather than once at construction — `load()`/hydration/
`put()`/the summary log are each individually skipped when it reads false, so
turning it off is a genuine no-op (nothing is read from or written to the
shared file), not just a hidden affordance. The setting is read where the
class already pays the `settings.ts` → `vscode` import cost
(`SessionDiscovery`, `ForeignWorkspaceManager`, `SiblingWorktreeManager`, all
of which already import `readSettings`); `replayCache.ts` and
`sessionPolling.ts` deliberately never import it, matching `replayCache.ts`'s
own note about the blast radius that would otherwise hit every test file
transitively importing `sessionManager.ts`.

`start()`'s preamble stage calls `replayCache.load()` right after
`meta.load()`, then independently re-reads and re-parses the same file (via
the exported `parseReplayCache()` — read-only, `ReplayCacheStore`'s public
interface has no enumeration method) purely to count and warn on any entry
whose key resolves outside `projectsDir` — defence against a hand-edited
cache file. Such an entry can never actually be used as a hydration source
regardless, since every `get()` this codebase issues is against a path it
itself constructed under `projectsDir`; the check is visibility/defence-in-
depth on top of that structural guarantee, not a functional gate. The same
pass also records the loaded entry count for the summary line below. Registry
scan ordering matters here too: `start()` runs the process-registry scan
before the local scan (see `start()`'s own docblock), so `livenessProbeFor()`
already has a real answer — not `null` — by the time the local scan's
hydration check runs; a session with a confirmed-live process must never
hydrate from a stale cached `done`.

Hydration happens at the same point in each scan that already stats the
candidate file, reusing that stat rather than taking a second one:
`SessionDiscovery.scanWorkspace()` for local sessions, and
`trackJsonlSessions()` (`sessionPolling.ts`) for foreign/sibling ones via its
optional `replayCache`/`livenessOf`/`makeHydrated` parameters — a manager
that hydrates skips its initial `update()` entirely (no tailer read at all),
and the sibling manager's `setWorktreeOrigin()` decoration is applied to a
hydrated manager exactly as it is to an ordinarily-constructed one. The local
scan factors its `SessionManagerOptions` literal into `managerOptsFor()` so
both construction paths (`new SessionManager` / `SessionManager.fromCache`)
share every decorator (status trace, bridge trace, probes, the
`registrySeenLive` latch) with no risk of the two drifting apart.

Population happens only at poll time, never at scan time (a scan cannot know
a >16 MB transcript's tailer has actually drained — see `checkMtime()`'s
doc comment above): `SessionDiscovery.pollInner()`'s dormant loop calls a
private `offerToCache()` right next to `sweepBackgroundWork()`, and
...
managers wire to the shared `offerSessionToReplayCache()` helper. That helper
— the one eligibility/write decision both call sites funnel through — skips
a manager that is itself hydrated (`isHydrated()`; nothing new to capture),
calls `exportCachedState(now)` and refuses on `null`, and skips the write
when the cache already holds an identical `{size, mtimeMs}` stamp for that
file. `replayCache.flush(now)` runs once, at the very end of `pollInner()`
(after the sibling/foreign/team/workflow polls, so it also picks up whatever
those polls' own `offer` calls just wrote this cycle) beside `meta.flush()`;
`stop()` fires a best-effort forced flush (`{force: true}`, bypassing only
the 30s rate limit, not the dirty check) beside the meta flush, so a session
that goes dormant just before the window closes isn't stranded until some
other window happens to offer it again.
...
its own timed stage. `N`/`M` sum the local scan's own counters with
`getReplayCacheStats()` from both cross-workspace managers; `K` is the entry
count `validateReplayCacheKeys()` recorded at load time. Skipped entirely
(not printed as an all-zero line) when the kill switch is off. A second
window (or VS Code profile, or farmed account) opening against the same disk
after the first one has gone quiet for `REPLAY_CACHE_QUIET_MS` therefore
hydrates most of its foreign/sibling sessions instead of replaying them —
see the `[startup] foreign scan` line's byte/session counts for the
before/after.
...
- **Tier 1 — completed (sidecar present), the common case.** `parseWorkflowSidecar()` reproduces the `/workflows` phase-grouped tree exactly: phase grouping, per-agent tokens/tools/duration/state, roll-up metrics, and `log()` narrator lines all read directly. Zero correlation, no heuristics. Sidecar-exists is **not** treated as terminal — see **Resumed runs** below.
- **Tier 2 — live (run dir exists, no sidecar yet).** Phase scaffold from the script's `meta.phases`; agents listed from the journal. Each running agent is correlated back to its phase by `extractAgentCalls()` (the static `agent(prompt, {label, phase})` call sites, never eval'd) matched against the agent's record-0 prompt: the longest distinctive *static* segment of a call's prompt that appears verbatim in the expanded prompt wins (interpolated `${...}` parts differ per agent, so we key off the static text around them), with **total matched length breaking ties** — sibling fan-out calls built from one helper share a long body segment, so the short substituted-arg segment (`project P2`) only discriminates via the sum, not the single longest. The prompt argument's shapes are all handled: a plain literal (segments lifted directly); a **concatenation embedding literals** (the `agent(COMMON + \`…\`, …)` shared-preamble shape — segments harvested from each embedded literal, since the const preamble is identical across agents and the per-call template is the distinctive part; the same scan skips over each literal so a comma or brace *inside* a template can't truncate the prompt arg or misplace the opts object); and **indirect expressions** resolved to virtual calls by `expandIndirectCalls()`. The indirect resolver covers the dominant real-world shape — a prompt built from an expression-bodied preamble helper — by **substituting the call's literal args into the helper's body template** (`agent(synthPrompt('P1'), …)`, and array elements `{ prompt: ev('cc-workspace', \`…\`) }` over `SOURCES.map`), so a substituted arg becomes a call-specific static segment (`=== YOUR SOURCE: cc-workspace ===`) rather than leaving every agent in the ungrouped bucket; a bare-identifier prompt (`agent(gapsPrompt, …)`) resolves to that const's template; block-bodied `fn(args)` and `c.prompt` array fan-outs of literal-prompt elements resolve as before. Only an expression with no embedded literal and no resolvable binding has no static segment to match, so it falls back to a flat, ungrouped entry with an agent-distinct label (the journal key is an implementation detail and never surfaces). The run-level header roll-up (agents · tokens · tools) is **summed from the per-agent live stats** — there is no on-disk total before the sidecar, so the header would otherwise read `0 tokens · 0 tools` while the per-agent rows showed real usage. Low-stakes either way: completion immediately materialises the perfect sidecar. Stale live runs that never produce a sidecar (killed/abandoned) are filtered by the 7-day age gate and marked `incomplete`.
  - **Parent-death gate, latched (fixed 2026-07-23).** A live run whose parent session is confirmed dead in a clean, active `processRegistry` scan is downgraded to `incomplete` at once, ahead of the age gate — but only once that session has been seen live at least once (`everSeenLiveSessions`), mirroring `SessionManager`'s `everSeenLiveInRegistry` death-gate. Real repro: a lead session idling between JSONL turns while its own background Workflow keeps running can read as absent from a single registry snapshot (the registry rescans only every 4th poll — `processRegistry.ts`) even though it's genuinely alive; an unlatched check misread that gap as death, flipped the run to `incomplete`, and — since `applyWorkflowLiveStatus` only pins a card on `status === 'running'` — left the session card stuck on `done` with the workflow still visibly running underneath. A session/run this probe never catches alive is never wrongly killed by it (same trade-off as the session-level gate); the 7-day age gate remains the backstop for a run that started dead before ever registering.
  - **Run-directory mtime pitfalls, fixed (2026-07-23).** A directory's own mtime only advances when an entry is added or removed inside it (a new sibling `agent-*.jsonl` appearing) — never when an existing file's content keeps growing. Two call sites read that mtime as if it meant "recent activity": the 7-day age-gate prune (a run idling with no new agents starting, but whose `journal.jsonl` was still being appended, could still get pruned as stale) and the live snapshot's `startTime` (a new sibling agent bumps the directory's mtime forward, so the reported run duration understates real elapsed time and creeps shorter over the run's life). Both now read `Math.max(directory mtime, journal.jsonl mtime)` for the age gate, and the directory's stable `birthtimeMs` (immutable on creation; falls back to mtime only if the filesystem doesn't report one) for `startTime`.
  - **Unconfirmed-liveness ceiling (added 2026-07-23).** The parent-death gate above only fires when the registry can actively vouch for a session being dead; a run whose session was never confirmed live, or whose registry scan is empty/degraded, rides `running` indefinitely — no different from a session's own `EXTENDED_THINKING_CEILING_MS` (15 min) backstop, which workflows lacked. A live run with no genuine journal activity for `UNCONFIRMED_LIVENESS_CEILING_MS` (30 min, more generous than the session ceiling since a fan-out phase can legitimately sit quiet longer) **and** no positive registry confirmation of its parent is now also downgraded to `incomplete`. A run whose parent the registry actively confirms live is exempt regardless of how long it's been quiet — registry evidence always outranks the inactivity clock.
  ...
  - **Resumed runs (fixed 2026-07-23).** The Workflow tool supports `resumeFromRunId`: an already-COMPLETED run can be relaunched under the SAME `runId`, replaying cached agents and running new ones. `workflows/wf_<runId>.json` is written once more only when the *resumed* run itself finishes — until then it sits stale on disk while `journal.jsonl` and the per-agent transcripts keep growing underneath it. Sidecar-exists is therefore not a terminal signal: `scanLiveRuns` compares the journal's last-activity mtime (`Math.max(run-dir mtime, journal.jsonl mtime)`, the same signal the age gate uses) against the sidecar's own completion evidence — its `timestamp` field when present (`workflowSidecar.ts:parseSidecarCompletedAt`, epoch ms parsed from the ISO string) else the sidecar file's own mtime — with `RESUME_JOURNAL_TOLERANCE_MS` (2s) slack absorbing filesystem timestamp jitter between two near-simultaneous writes. Journal newer than the sidecar by more than the tolerance re-engages the live tier exactly as for a never-completed run (fresh `JsonlTailer` from byte 0 — safe and cheap, since the journal is one continuous, cumulative file across resume cycles); the card reflects live agents again. When the resumed run itself completes, Claude Code rewrites the sidecar with a newer mtime/timestamp than the (now quiet) journal, and the run settles back to Tier 1 showing the new, larger tree. If a resumed run goes quiet past the **same** liveness window that would downgrade a never-completed run (the parent-death gate or the unconfirmed-liveness ceiling above), it is classified as an **abandoned resume**: rather than surface that synthetic `incomplete` roster, the card falls back to the last completed sidecar tree (`sidecarEvidence`, held independently of the currently-displayed snapshot precisely so this fallback survives a live-tier overwrite) — the sidecar is a truthful, complete artefact for the run's last finished cycle, whereas a partial live reconstruction of an abandoned resume is not. Real repro, 2026-07-23: session `0d045c02-…`'s `wf_64c6bbb3-4e2` completed at 10 agents, was resumed ~6 minutes later, and grew to 46 agents across the resume before its sidecar was rewritten — the panel showed the stale 10-agent "finished" tree for the whole resumed run under the old sidecar-exists-is-terminal assumption. **Abandoned-resume wake-up (fixed 2026-07-23).** The fallback keeps its own `state.lastChangedAt` current so a later genuine wake-up (real journal growth) can clear the unconfirmed-liveness ceiling and re-enter the live tier — but it compares a **content-only** signature (run `status` excluded), not `liveSnapshotSig`'s full one: `status` is itself derived from `lastChangedAt` via the ceiling, so comparing the full signature is circular — the ceiling flipping `status` to `incomplete` would itself read as "activity", refresh the clock, and un-flip `status` back to `running` on the very next scan with zero real journal growth (confirmed empirically; the plain live path has the identical latent quirk, undisturbed here). **Sidecar size cap (raised 2026-07-23).** `SIDECAR_MAX_BYTES` matches `JOURNAL_MAX_BYTES` (4 MB, was 1 MB) since a resume's completion rewrite is strictly larger than its predecessor; if a rewrite still lands oversized, `scanSidecars` refreshes the run's freshness evidence from the stat (mtime only — the content is never read) so the run still settles to the last sidecar tree it *could* parse rather than riding `running` forever. **Cross-session resume (watch item, unhandled).** All of the above assumes the resume is observed by the same `WorkflowDiscovery` instance that saw the original completion (`sidecarEvidence` is in-memory, per workspace). A resume executed from a different session — if the Workflow tool supports that; unverified — would leave the process-death/parent-liveness checks keyed to the ORIGINAL session, potentially tripping the parent-death gate and pinning the stale sidecar tree while the new session's journal grows underneath it undetected.

### Status mapping

The sidecar's per-agent `state` maps to `WorkflowAgentStatus` (`DisplayStatus` plus `failed` — preserved so the detail panel can sort failed agents first and roll them up); `error` maps to `failed`, and an **unknown state degrades to `done`, never `running`** — a completion artifact must not leave phantom running agents feeding the card's live-agent count (the v1.16.21 ghost bug). An agent entry with a null `agentId` (the runtime writes this for an agent that errored before registering) keeps its row under a synthesised `missing-<n>` id rather than being dropped, so roll-ups don't undercount. Run `status` maps to `WorkflowRunStatus` (`completed` / `running` / `failed` / `incomplete`); the sidecar's `error` field (message + stack) is parsed into `WorkflowSnapshot.error` and surfaced by the detail header (a terminal-bad `runStatus` overrides the agent-derived pill, and a failed 0-agent run shows the error line instead of the bare empty-roster copy). Sidecar roll-up numbers are preferred over recomputation (retries are surfaced via `attempt`).
