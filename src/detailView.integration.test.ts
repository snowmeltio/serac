...
describe('detailView.ts — transcript delivery', () => {
  beforeEach(async () => {
    postedMessages = [];
  ...
  it('agentTranscriptAppend renders the appended entries', () => {
    sendRender(twoSourceModel());
    sendTranscript(KEY, [
      { timestamp: '2026-06-10T10:00:00Z', role: 'user', content: 'the brief' },
      { timestamp: '2026-06-10T10:00:05Z', role: 'assistant', content: 'first reply', kind: 'text' },
    ]);
    expect(q('.wf-log')!.textContent).toContain('first reply');

    sendTranscriptAppend(KEY, [{ timestamp: '2026-06-10T10:00:10Z', role: 'assistant', content: 'second reply', kind: 'text' }]);
    const log = q('.wf-log')!;
    expect(log.textContent).toContain('first reply');
    expect(log.textContent).toContain('second reply');
  });
  ...
  it('append replaces the inbox suffix wholesale (it can shrink)', () => {
    ...
    sendTranscript(KEY, [
      { timestamp: '2026-06-10T10:00:00Z', role: 'user', content: 'the brief' },
      { timestamp: '2026-06-10T10:00:05Z', role: 'assistant', content: 'reply', kind: 'text' },
    ], [{ timestamp: '2026-06-10T10:00:06Z', role: 'system', content: 'Queued for delivery (from murray): hello' }]);
    expect(q('.wf-log')!.textContent).toContain('hello');

    // Teammate drained its inbox: empty suffix, no new entries.
    sendTranscriptAppend(KEY, [], []);
    expect(q('.wf-log')!.textContent).not.toContain('hello');
    // The real turns survive untouched.
    expect(q('.wf-log')!.textContent).toContain('reply');
  });

  it('pins the brief row once the first user entry arrives in a delta', () => {
    sendRender(twoSourceModel());
    // No user entry yet — nothing is pinned as the brief.
    sendTranscript(KEY, [{ timestamp: '2026-06-10T10:00:00Z', role: 'assistant', content: 'working', kind: 'text' }]);
    expect(q('.wf-log-row.brief')).toBeNull();

    sendTranscriptAppend(KEY, [{ timestamp: '2026-06-10T10:00:05Z', role: 'user', content: 'late brief' }]);
    expect(q('.wf-log-row.brief')!.textContent).toContain('late brief');
  });
  ...
  it('re-requests a full snapshot when an append arrives with no cached anchor', () => {
    ...
    sendTranscriptAppend(KEY, [{ timestamp: 't', role: 'assistant', content: 'x' }]);
  ...
  it('does not pin a tool-role entry as the inception brief', () => {
    ...
    expect(q('.wf-log-row.brief')!.textContent).toContain('the real brief');
  });
});
...
describe('detailView.ts — live transcript refresh', () => {
  ...
  it('re-requests a running agent’s transcript on the steady interval', () => {
    ...
    qa('.wf-agent-pill').find(p => p.dataset.agent === 'agent002')!.click(); // select the running one
...
describe('detailView.ts — UX batch (dedup, persistence, chip summary)', () => {
  beforeEach(async () => {
    postedMessages = [];
    webviewState = undefined;
    document.body.innerHTML = '<div id="wf-root"></div>';
  ...
  it('an identical steady-tick transcript does NOT re-render (text selection survives)', () => {
    ...
    expect(q('.wf-log-scroll')).toBe(scrollBefore); // same node — no innerHTML swap
  });
  ...
  it('a grown transcript still re-renders with the new turn', () => {
    ...
    sendTranscript(KEY, T1);
    const scrollBefore = q('.wf-log-scroll');
    sendTranscript(KEY, [...T1, { timestamp: '2026-06-10T01:00:10Z', role: 'assistant', content: 'second reply', kind: 'text' }]);
  ...
  it('does NOT restore selection into a different drill-in (falls back to first agent)', async () => {
    ...
    sendRender(other);
    expect(q('.wf-agent-pill.active')!.dataset.agent).toBe('agent001');
  });
...
describe('detailView.ts — log view (Phase 2, default mode)', () => {
  ...
  it('the log scroll container is keyboard-focusable, with the jump pill hidden at the bottom', () => {
    sendRender(logModel());
    sendTranscript(KEY1, [
      { timestamp: '2026-06-10T10:00:00Z', role: 'user', content: 'the brief' },
      { timestamp: '2026-06-10T10:00:05Z', role: 'assistant', content: 'reply', kind: 'text' },
    ]);
    expect(q('.wf-log-scroll')!.getAttribute('tabindex')).toBe('0');
    // jsdom has no layout (clientHeight/scrollHeight both 0), which reads as
    // "at the bottom" — the pill exists but stays hidden.
    expect(q('.wf-jump-latest')).not.toBeNull();
    expect(q('.wf-jump-latest')!.classList.contains('visible')).toBe(false);
  });

  it('scrolling up reveals the jump-to-latest pill; clicking it returns to the tail and hides it', () => {
    sendRender(logModel());
    sendTranscript(KEY1, [
      { timestamp: '2026-06-10T10:00:00Z', role: 'user', content: 'the brief' },
      { timestamp: '2026-06-10T10:00:05Z', role: 'assistant', content: 'reply', kind: 'text' },
    ]);
    const scroll = q('.wf-log-scroll')!;
    // Simulate a laid-out, scrolled-up log (jsdom has no layout engine).
    Object.defineProperty(scroll, 'scrollHeight', { configurable: true, get: () => 1000 });
    Object.defineProperty(scroll, 'clientHeight', { configurable: true, get: () => 200 });
    scroll.scrollTop = 0;
    scroll.dispatchEvent(new Event('scroll'));
    const pill = q('.wf-jump-latest')!;
    expect(pill.classList.contains('visible')).toBe(true);

    pill.click();
    expect(scroll.scrollTop).toBe(1000); // jumped to the tail
  ...
  it('zones stack view row, then header strip (summary), then agent strip, then the rest', () => {
    ...
    // The trailing jump pill is absolutely positioned (out of flow), but it
    // is still a flow-order child after the scroll container.
    const order = ['wf-view-row', 'wf-hstrip', 'wf-agentstrip', 'wf-permrow', 'wf-rstrip', 'wf-facets', 'wf-log-scroll', 'wf-jump-latest'];
