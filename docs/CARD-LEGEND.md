...
| **Context bar** | Fill toward the auto-compact threshold |
...
**Context bar** — how full the context window is, measured against the **auto-compact threshold** rather than the model's raw capacity, since the threshold is what actually interrupts the session. Turns peach past 60%. Hover for both numbers.
