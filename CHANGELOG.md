...
- **Rows wait for their repo root before painting.** A workspace whose sessions were tracked but whose repo root was still unresolved painted as an unfolded row with no stale promotion whenever an update landed mid-scan; since v1.26.0's progressive first paint that included every window start. Such a key is now held back until the same scan resolves it, at most one render later.

## v1.26.0 (2026-09-10) — Faster new windows, and cards that stay put

### Fixed
- **A card you never archived no longer vanishes at day seven.** The local scan's seven-day age gate hid any transcript older than that, while the foreign-workspace badge (fourteen-day gate) still counted it, so the hub workspace showed two cards against a badge count of eleven, with 32 undismissed sessions invisible. Sessions the active scan has loaded are now stamped `tracked` in session-meta.json, and an undismissed tracked session loads regardless of file age. Entries that exist only from the title backfill stay untracked, so never-seen dormant transcripts stay off the list. Un-archiving a card brings it back on the next scan.
- **Transcripts over 16MB no longer stall half-read.** The tailer reads 16MB per pass, and the dormant check compared mtime only, so a large transcript classified dormant right after its first pass never got a second one. One update now drains the whole file, the dormant check also fires while unread bytes remain, and the record straddling each 16MB boundary, which the line-buffer guard was silently dropping, is kept.

### Changed
- **Opening a new window is faster.** Measured on a workspace with 198 transcripts (537MB) in the discovery window, 195 of them quiet for over ten minutes: nearly all startup work was replaying dormant sessions that had not changed since another window last read them. Three changes: (1) the panel paints progressively, with a phase flag (pending, partial, ready) and a trailing throttle, instead of waiting on every discovery stage before the first card; (2) the sibling-worktree classifier reads a bounded 1MB head of a candidate transcript for its `cwd` instead of replaying the whole file through a throwaway manager; (3) a shared replay cache, `serac-replay-cache.json` beside the projects directory, records the settled state of finished, quiet sessions so the next window (or profile, or farmed account, since the file sits on the realpath every window shares) hydrates them without replaying from byte zero. Any change to a cached transcript's size or mtime forces a full replay before live records are applied. Multiple windows write the file with tmp-then-rename and merge before every write; the race is narrowed, not closed. `serac.discovery.replayCache` (default on) is a genuine kill switch for both reading and writing, taking effect on the next reload. The `[startup]` and `[replay-cache]` output-channel lines carry the timings; no before-and-after figure is claimed here until they are measured end-to-end.
...
### Added
- **A 📡→ chip on phone-started sessions brings them into this window (experimental).** v1.24.0 moved Remote Control servers to same-dir so a phone-started session lands under this workspace's project key, and the Claude Code panel still declined to restore it: the panel host hard-codes `includeProgrammaticSessions = false`, and its lister drops every transcript whose records say `entrypoint: sdk-cli`, which is how every Remote Control-hosted process writes. No key or folder arrangement gets past that. Two spikes settled the workaround: rewriting the value to `claude-vscode` on every record makes the same file restorable, and a plain SIGTERM to the idle `sdk-cli` child is a clean release (gone in about two seconds, registry entry cleared, `claude rc` server untouched and no respawn, one ordinary closing record). With `serac.experimental.transferPhoneSessions` on, cards whose transcript reads `sdk-cli` carry a dish-and-arrow chip. Clicking it: if the phone's process is alive and idle, a modal asks first, then Serac terminates it (never SIGKILL; a five-second timeout warns and touches nothing) and waits for its registry entry to clear; the transcript is streamed to a same-directory temp file with every `"entrypoint":"sdk-cli"` token rewritten, one `serac-transfer` record is appended naming the original value, and the file is renamed into place; Serac's own tailer re-reads the grown file; the chat opens here, and this window's Remote Control auto-enrol puts it back on the phone under a new bridge id. Mid-turn on the phone (Serac still sees a turn, or the transcript was written in the last eight seconds) the click is refused with a status-bar message. While the gate is on, the card body of such a session is a no-op with a hint, because the only thing it could do is open an empty chat; the transcript and dismiss buttons keep working. Off by default: no chip, no rewrite, and clicks behave as before. The "entrypoint" value is now read from the transcript into each session's snapshot (most recent record wins, so the chip clears itself after a transfer). The upstream ask, that Remote Control sessions be exempt from the panel's programmatic filter, stands; this is the workaround until then.

## v1.24.0 (2026-09-02) — Phone sessions you can open here

### Changed
- **Remote Control servers now start in same-dir mode.** Every session your phone started ran in a one-shot `<repo>/.claude/worktrees/bridge-cse_*` worktree, so its transcript lived under that worktree's project key — and the Claude Code panel restores a session only from the open folder's own key. Clicking such a card got you a fresh chat (the extension logs say `restore_declined`), whether the session was live or long dead. Serac's "Start a Remote Control server here" and the launchd installer now run plain `claude rc` (Claude Code's own `--spawn same-dir` default): phone sessions share the checkout, land under this workspace's key, show as ordinary local cards, and open in the panel on click. Worktree isolation is the opt-in: `serac.rc.spawnMode` (per-workspace) is `same-dir`, `worktree` (always, the CLI's own error outside a git repo), or `auto` (match the workspace: worktree where it is a git repo, same-dir otherwise). A Drive-synced folder that is not a repo used to fail on `git worktree` before it served anything. The launchd wrapper takes the mode as its second argument; an agent installed before this release passes none and gets same-dir on its next restart. Transcripts already sitting under a `bridge-cse_*` key stay unopenable from the panel — the scroll icon still renders them.
- **The Remote Control signal bars are easier to read.** An empty bar was a 1px outline that, at 3px wide, read as the darkest stroke in the top bar and out-competed the lit fill beside it. Empty bars are now a faint solid ghost of the theme's description colour, the bars are a touch larger, and the lit fill uses the done-accent token so it keeps its contrast on a light theme.

## v1.23.1 (2026-09-01) — Folded cards open here
...
### Added
- **The Remote Control start routes now only fire in the default profile.** The phone's Remote Control view is per account, so a server started from a companion-profile window (a `code-<name>` launcher's account dir and instance dir) serves capacity your phone cannot reach without switching accounts there — the offer was a dead end. In a companion profile the picker's two server rows (terminal server and launchd installer) are withheld; when nothing offerable remains, no picker opens and the reason lands in the status bar and the indicator's aria label. Observability is untouched everywhere: the signal bar level still reports the facts as they are, and the companion tooltip gains a caveat about this window's account — deliberately never a claim about who owns a detected server, since the serving fact is account-blind. "Default" requires both signals to agree (no `CLAUDE_CONFIG_DIR` beyond `~/.claude`, and a recognised editor instance dir — remote-development server layouts included); anything unrecognised fails closed, and the verdict is logged once per window to the Serac output channel. Hard rule, no setting.

### Fixed
- **A writer whose owning window died no longer keeps a stale handoff address.** Ownership verdicts were resolved once per process and never re-checked, so the recorded owner pid could outlive the owner — and, if the OS recycled that pid onto another editor process, a cross-window focus hint could be addressed to a window that never owned the session. Non-own verdicts now re-resolve once a minute so the address tracks the writer's real parent. A failed re-check keeps the standing verdict without tightening the cadence, and the `ps` probe now always settles, so a wedged probe can no longer hang every open decision behind it.
